...
using SparkleAir.Infa.Dto.Payment.Models;
...
namespace SparkleAir.Front.API.Controllers.Payment
{
    ...
    public class ECPayPaymentsController : ControllerBase
    {
        ...
        public ECPayPaymentsController(ILogger<ECPayPaymentsController>  logger, AppDbContext db)
        {
        ...
        [HttpGet("Index")]
        public async Task<List<OrderDetail>> Index(string TotalAmount, string ItemName, string Url)
        {
            ...
            var order = new List <OrderDetail>
{
    ...
    new OrderDetail { key="OrderResultURL", data= $"{website1}/api/ECPayPayments/{Url}"},
    ...
    new OrderDetail { key="ClientRedirectURL", data= $"{website2}/Pay/milePurchaseTest"},
        ...
        public async Task<IActionResult> GetMileOrder([FromForm] IFormCollection form)
        {
            try
            {
                // 提取表單數據中的相關信息
                ...
                return Redirect($"http://127.0.0.1:8888/MileOrderPayoutSuccess/{paymentId}");
            }
            catch (Exception ex)
            {
                // 如果發生異常，記錄錯誤並返回錯誤狀態碼
        ...
        public async Task<IActionResult> GetLuggageOrder([FromForm] IFormCollection form)
        {
            try
            {
                // 提取表單數據中的相關信息
                ...
                return Redirect($"http://127.0.0.1:8888/LuggageOrderPayoutSuccess/{paymentId}");
            }
            catch (Exception ex)
            {
                // 如果發生異常，記錄錯誤並返回錯誤狀態碼
        ...
        public async Task<IActionResult> GetTicketOrder([FromForm] IFormCollection form)
        {
            try
            {
                // 提取表單數據中的相關信息
                ...
                return Redirect($"http://127.0.0.1:8888/TicketOrderPayoutSuccess/{paymentId}");
            }
            catch (Exception ex)
            {
                // 如果發生異常，記錄錯誤並返回錯誤狀態碼
        ...
        public async Task<IActionResult> GetTaxfreeOrder([FromForm] IFormCollection form)
        {
            try
            {
                // 提取表單數據中的相關信息
                ...
                return Redirect($"http://127.0.0.1:8888/TaxfreeOrderPayoutSuccess/{paymentId}");
            }
            catch (Exception ex)
            {
                // 如果發生異常，記錄錯誤並返回錯誤狀態碼
